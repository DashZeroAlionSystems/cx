...
namespace CX.Container.Server.Controllers.CX
{
    [Authorize]
    [ApiController]
    [Route("api/config/text-to-schema-assistants")]
    [ApiVersion("1.0")]
    public sealed class TextToSchemaAssistantsConfigController(
        IServiceProvider sp,
        IOptionsMonitor<StructuredDataOptions> structuredDataOptions,
        ACLService aclService) : ControllerBase
    {
        private readonly JsonStore configStore = new("config_text_to_schema_assistants", 100, pg_default, sp);

        [HttpGet]
        [RequiresAtLeastUserRole]
        [Route("")]
        public async Task<ActionResult<IEnumerable<TextToSchemaOptions>>> GetConfigAsync()
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<TextToSchemaOptions>> GetConfigByIdAsync(string id)
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<TextToSchemaOptions>> CreateConfigAsync(string id, [FromBody] JsonElement config)
        {
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest(new { Message = $"{nameof(id)} must not be empty" });

            await configStore.SetAsync<JsonElement>(id, config);
            return NoContent();
        }
