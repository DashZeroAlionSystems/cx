...
namespace CX.Container.Server.Controllers.CX
{
    [Authorize]
    [ApiController]
    [Route("api/config/jsonschemas")]
    [ApiVersion("1.0")]
    public sealed class JsonSchemasConfigController(
        IServiceProvider sp,
        IOptionsMonitor<StructuredDataOptions> structuredDataOptions,
        ACLService aclService) : ControllerBase
    {
        private readonly JsonStore configStore = new("config_jsonschemas", 100, pg_default, sp);

        [HttpGet]
        [AllowAnonymous]
        [Route("")]
        public async Task<ActionResult<IEnumerable<JsonStoreOptions>>> GetConfigAsync()
        {
        ...
        [Route("{id}")]
        public async Task<ActionResult<JsonStoreOptions>> GetConfigByIdAsync(string id)
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<JsonStoreOptions>> CreateConfigAsync(string id, [FromBody] JsonElement config)
        {
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest(new { Message = $"{nameof(id)} must not be empty" });
