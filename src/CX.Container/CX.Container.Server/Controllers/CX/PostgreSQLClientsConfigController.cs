...
namespace CX.Container.Server.Controllers.CX
{
    [Authorize]
    [ApiController]
    [Route("api/config/postgresqlclients")]
    [ApiVersion("1.0")]
    public sealed class PostgreSQLClientsConfigController(
        IServiceProvider sp,
        IOptionsMonitor<StructuredDataOptions> structuredDataOptions,
        ACLService aclService) : ControllerBase
    {
        private readonly JsonStore configStore = new("config_postgresqlclients", 100, pg_default, sp);

        [HttpGet]
        [RequiresAtLeastUserRole]
        [Route("")]
        public async Task<ActionResult<IEnumerable<PostgreSQLClientOptions>>> GetConfigAsync()
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<PostgreSQLClientOptions>> GetConfigByIdAsync(string id)
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<PostgreSQLClientOptions>> CreateConfigAsync(string id, [FromBody] PostgreSQLClientOptions config)
        {
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest(new { Message = $"{nameof(id)} must not be empty" });
