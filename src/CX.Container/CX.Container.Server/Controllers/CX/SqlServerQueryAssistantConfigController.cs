...
namespace CX.Container.Server.Controllers.CX
{
    [Authorize]
    [ApiController]
    [Route("api/config/sql-assistants")]
    [ApiVersion("1.0")]
    public sealed class SqlServerQueryAssistantConfigController(
        IServiceProvider sp,
        IOptionsMonitor<StructuredDataOptions> structuredDataOptions,
        ACLService aclService) : ControllerBase
    {
        private readonly JsonStore configStore = new("config_sql_server_query_assistants", 100, pg_default, sp);
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<SqlServerQueryAssistantOptions>> CreateConfigAsync(string id, [FromBody] SqlServerQueryAssistantOptions config)
        {
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest(new { Message = $"{nameof(id)} must not be empty" });
