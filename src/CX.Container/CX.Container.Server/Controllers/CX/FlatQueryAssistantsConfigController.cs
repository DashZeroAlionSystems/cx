...
namespace CX.Container.Server.Controllers.CX
{
    [Authorize]
    [ApiController]
    [Route("api/config/flatqueryassistants")]
    [ApiVersion("1.0")]
    public sealed class FlatQueryAssistantsConfigController(
        IServiceProvider sp,
        IOptionsMonitor<StructuredDataOptions> structuredDataOptions,
        ACLService aclService) : ControllerBase
    {
        private readonly JsonStore configStore = new("config_flatqueryassistants", 100, pg_default, sp);

        [HttpGet]
        [RequiresAtLeastUserRole]
        [Route("")]
        public async Task<ActionResult<IEnumerable<FlatQueryAssistantOptions>>> GetConfigAsync()
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<FlatQueryAssistantOptions>> GetConfigByIdAsync(string id)
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<FlatQueryAssistantOptions>> CreateConfigAsync(string id, [FromBody] FlatQueryAssistantOptions config)
        {
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest(new { Message = $"{nameof(id)} must not be empty" });
