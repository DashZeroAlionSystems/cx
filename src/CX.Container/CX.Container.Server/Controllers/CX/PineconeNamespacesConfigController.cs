...
namespace CX.Container.Server.Controllers.CX
{
    [Authorize]
    [ApiController]
    [Route("api/config/pinecone-namespaces")]
    [ApiVersion("1.0")]
    public sealed class PineconeNamespacesConfigController(
        IServiceProvider sp,
        IOptionsMonitor<StructuredDataOptions> structuredDataOptions,
        ACLService aclService) : ControllerBase
    {
        private readonly JsonStore configStore = new("config_pinecone_namespaces", 100, pg_default, sp);

        [HttpGet]
        [RequiresAtLeastUserRole]
        [Route("")]
        public async Task<ActionResult<IEnumerable<PineconeNamespaceOptions>>> GetConfigAsync()
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<PineconeNamespaceOptions>> GetConfigByIdAsync(string id)
        {
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<PineconeNamespaceOptions>> CreateConfigAsync(string id, [FromBody] PineconeNamespaceOptions config)
        {
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest(new { Message = $"{nameof(id)} must not be empty" });
