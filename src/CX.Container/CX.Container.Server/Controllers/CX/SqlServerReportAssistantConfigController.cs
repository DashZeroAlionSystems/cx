...
namespace CX.Container.Server.Controllers.CX
{
    [Authorize] 
    [ApiController]
    [Route("api/config/sql-report-assistants")]
    [ApiVersion("1.0")]
    public sealed class SqlServerReportAssistantConfigController(
        IServiceProvider sp,
        IOptionsMonitor<StructuredDataOptions> structuredDataOptions,
        ACLService aclService) : ControllerBase
    {
        private readonly JsonStore configStore = new("config_sql_server_report_assistants", 100, pg_default, sp);
        ...
        [RequiresAtLeastUserRole]
        [Route("{id}")]
        public async Task<ActionResult<SqlServerReportAssistantOptions>> CreateConfigAsync(string id, [FromBody] SqlServerReportAssistantOptions config)
        {
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest(new { Message = $"{nameof(id)} must not be empty" });
